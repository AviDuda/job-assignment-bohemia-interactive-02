...
export default function Cart({ onCartOpenChange }: CartProps) {
    ...
    return (
        <div
            className="fixed left-0 top-24 z-modal flex h-[calc(100%-6rem)] w-full flex-col gap-6 overscroll-contain border-4 border-t-[1px] border-gray-200 bg-white px-6 py-7 dark:border-zinc-700 dark:bg-zinc-900 sm:sticky sm:left-full sm:h-[inherit] sm:max-h-max sm:max-w-max lg:top-32"
            ref={cartRef}
        >
            <a href="#" onClick={onCartOpenChange} className="self-end">
                <SvgCloseIcon />
            </a>
            <div className="flex flex-col gap-8 overflow-scroll border-b-2 border-b-gray-300 pb-8 dark:border-b-zinc-700 sm:overflow-auto">
                {cartEntries.length === 0 && (
                    <div className="text-xl font-bold text-gray-500 dark:text-zinc-400">Ready to go shopping?</div>
                )}
